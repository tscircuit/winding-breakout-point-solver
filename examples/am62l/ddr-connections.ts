...
export interface DdrConnection {
  ...
  readonly busName: DdrBusName
}

/**
 * Hand-authored electrical intent for the example. Component geometry,
 * source-port locations, and breakout points are all rendered by core.
 */
...
export const DDR_BUS_ORDER = [
  "DDR_BYTE1",
  "DDR_ADDR_CTRL",
  "DDR_BYTE0",
] as const satisfies readonly DdrBusName[]

export const getTraceNamesForBus = (busName: DdrBusName): string[] =>
  DDR_CONNECTIONS.filter((connection) => connection.busName === busName).map(
